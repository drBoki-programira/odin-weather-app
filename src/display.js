...
function makeBottomRow(parent, uv, wd, ws, rp, unit) {
  ...
  const rpIconContainer = makeChildOf(rpDiv, "div", {"className": "img-container"})
  ...
  const wsIconContainer = makeChildOf(wsDiv, "div", {"className": "img-container"})
...
export function updateDisplay(data) {
  ...
  makeChildOf(todayInfo, "div", {"className": "city-name", "textContent": capitalize(data.city)})
  ...
  makeBottomRow(todayInfo, data.uvIndex, data.windDirection, data.windSpeed, data.rainProb, data.unit)
  ...
  for (let i = 1; i < 4; i++) {
    ...
    makeBottomRow(box, data[`day${i}uv`], data[`day${i}wd`], data[`day${i}ws`], data[`day${i}rp`], data.unit)
  }
}
...
export function displayError(input) {
  ...
  makeChildOf(main, "div", {"className": "error", "textContent": `Oops, could't find ${input}. Please try again`})
}
...
export function loadingScreen() {
  ...
  makeChildOf(main, "div", {"className": "loading", "textContent": `Loading ...`})
}